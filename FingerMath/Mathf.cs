namespace FingerMath
{
    ...
    public static class Mathf
    {
        ...
        {
            var num = Repeat(target - current, 2 * (float)Math.PI);
            if (num > Math.PI)
                num -= 2 * (float)Math.PI;
        ...
        public static float ApproachAngleRadians(float start, float end, float shift)
        {
            ...
            if (-shift < deltaAngleRadians && deltaAngleRadians < shift)
                return end;
            return Repeat(Approach(start, start + deltaAngleRadians, shift), (float)Math.PI*2);
        }
        ...
        {
            var rounded = (float)Math.Round(value);
            roundedAmount = value - rounded * (float)Math.Round(value / rounded);
            return rounded;
        }
        ...
        {
            return (float)Math.Round(value / roundToNearest) * roundToNearest;
        }
